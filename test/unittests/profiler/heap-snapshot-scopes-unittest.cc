...
class WithHeapSnapshot : public TMixin {
 public:
  ...
  v8::Local<v8::Value> RunJS(const char* source) {
    return TMixin::RunJS(source);
  }

  template <typename T = JSFunction>
  DirectHandle<T> RunJSForObject(const char* source) {
    v8::Local<v8::Value> val = TMixin::RunJS(source);
  ...
  v8::Local<v8::Module> RunModule(const char* source) {
    v8::Local<v8::String> source_string =
        v8::String::NewFromUtf8(this->isolate(), source).ToLocalChecked();
    v8::ScriptOrigin origin(
        v8::String::NewFromUtf8(this->isolate(), "module.js").ToLocalChecked(),
        0, 0, false, -1, v8::Local<v8::Value>(), false, false, true);
    v8::ScriptCompiler::Source script_source(source_string, origin);
    v8::Local<v8::Module> module =
        v8::ScriptCompiler::CompileModule(this->isolate(), &script_source)
            .ToLocalChecked();
    CHECK(module->InstantiateModule(this->context(), ResolveModuleCallback)
              .FromJust());
    v8::MaybeLocal<v8::Promise> promise = module->Evaluate(this->context());
  ...
  DirectHandle<T> GetModuleExport(v8::Local<v8::Module> module,
    ...
    v8::Local<v8::Value> export_val =
        ns->Get(this->context(),
                v8::String::NewFromUtf8(this->isolate(), export_name)
                    .ToLocalChecked())
            .ToLocalChecked();
  ...
  SnapshotScopeId GetScopeId(Tagged<JSFunction> function) {
    const HeapEntry* entry =
        GetEntryFor(TMixin::i_isolate(), snapshot_, function);
  ...
  void CheckContextSlots(Tagged<JSFunction> function) {
    ...
    for (Tagged<Context> ctx = function->context();
      ...
      EXPECT_EQ(curr_scope->variables.size(), static_cast<size_t>(local_count));

      // Check that the variables from the source scopes match exactly the order
      // of context fields in the corresponding ScopeInfo.
      ...
      const HeapEntry* ctx_entry =
          GetEntryFor(this->i_isolate(), snapshot_, ctx);
      ...
      for (size_t i = 0; i < context_var_edges.size(); ++i) {
        EXPECT_EQ(curr_scope->variables[i].name, context_var_edges[i]->name());
      }
      if (!IsNativeContext(ctx->previous())) {
        ...
        const HeapEntry* prev_entry =
            GetEntryFor(this->i_isolate(), snapshot_, ctx->previous());
...
using HeapSnapshotScopesTest = WithHeapSnapshot<TestWithContext>;
